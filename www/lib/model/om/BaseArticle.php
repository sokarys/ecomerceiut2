...
abstract class BaseArticle extends BaseObject  implements Persistent {
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Article has previously been saved, it will retrieve
	 * related LigneCommandes from storage. If this Article is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collLigneCommandes === null) {
			if ($this->isNew()) {
			   $this->collLigneCommandes = array();
				...
				$criteria->add(LigneCommandePeer::ARTICLE_ID, $this->id);
	 ...
	 * actually need in Article.
	 */
	public function getLigneCommandesJoinCommande($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ArticlePeer::DATABASE_NAME);
		}
		...
		if ($this->collLigneCommandes === null) {
			if ($this->isNew()) {
				...
				$this->collLigneCommandes = LigneCommandePeer::doSelectJoinCommande($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
